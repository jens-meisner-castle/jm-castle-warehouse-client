...
export const useArticleSelect = (
  ...
  const [queryStatus, setQueryStatus] = useState<
    | ApiServiceResponse<{
        ...
        pages: SelectResponse<Row_Article>[];
        finished: boolean;
      }>
  ...
  useEffect(() => {
    ...
    const articleResponse = response?.length ? response[0] : undefined;
  ...
  useEffect(() => {
    if (currentPage >= 0 && typeof totalCount === "number") {
      ...
      const url = `${apiUrl}/article/page/select?page=${currentPage}&page_size=${pageSize}`;
      fetch(url, options)
        .then((response) => {
          response
            .json()
