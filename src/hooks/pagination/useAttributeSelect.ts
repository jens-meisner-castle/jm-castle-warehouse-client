...
export const useAttributeSelect = (
  ...
  const [queryStatus, setQueryStatus] = useState<
    | ApiServiceResponse<{
        ...
        pages: SelectResponse<Row_Attribute>[];
        finished: boolean;
      }>
  ...
  useEffect(() => {
    ...
    const attributeResponse = response?.length ? response[0] : undefined;
    const { result } = attributeResponse || {};
    const { row } = result || {};
  ...
  useEffect(() => {
    if (currentPage >= 0 && typeof totalCount === "number") {
      ...
      const url = `${apiUrl}/attribute/page/select?page=${currentPage}&page_size=${pageSize}`;
      fetch(url, options)
        .then((response) => {
          response
            .json()
