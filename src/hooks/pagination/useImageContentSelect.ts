...
import { useImageContentPage } from "./useImageContentPage";

const tables = ["image_content"];
...
export const useImageContentSelect = (
  ...
  const [queryStatus, setQueryStatus] = useState<
    | ApiServiceResponse<{
        ...
        pages: SelectResponse<Row_ImageContent>[];
        finished: boolean;
      }>
  ...
  useEffect(() => {
    ...
    const imageContentResponse = response?.length ? response[0] : undefined;
    const { result } = imageContentResponse || {};
    const { row } = result || {};
  ...
  const { data, isLoading } = useImageContentPage(
    apiUrl,
    currentPage,
    pageSize,
