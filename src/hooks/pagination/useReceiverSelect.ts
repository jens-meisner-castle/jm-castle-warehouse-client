...
import { useReceiverPage } from "./useReceiverPage";
...
export const useReceiverSelect = (
  ...
  const [queryStatus, setQueryStatus] = useState<
    | ApiServiceResponse<{
        ...
        pages: SelectResponse<Row_Receiver>[];
        finished: boolean;
      }>
  ...
  useEffect(() => {
    ...
    const receiverResponse = response?.length ? response[0] : undefined;
    const { result } = receiverResponse || {};
    const { row } = result || {};
  ...
  const { data, isLoading } = useReceiverPage(
    apiUrl,
    currentPage,
    pageSize,
