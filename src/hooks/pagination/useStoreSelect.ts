...
export const useStoreSelect = (
  ...
  const [queryStatus, setQueryStatus] = useState<
    | ApiServiceResponse<{
        ...
        pages: SelectResponse<Row_Store>[];
        finished: boolean;
      }>
  ...
  useEffect(() => {
    ...
    const storeResponse = response?.length ? response[0] : undefined;
  ...
  useEffect(() => {
    if (currentPage >= 0 && typeof totalCount === "number") {
      ...
      const url = `${apiUrl}/store/page/select?page=${currentPage}&page_size=${pageSize}`;
      fetch(url, options)
        .then((response) => {
          response
            .json()
